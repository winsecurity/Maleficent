...
impl peparser{
        ...
        unsafe{
            ...
            if self.filecontents.len()>1{
                ...
                for i in 0..exportdirectory.NumberOfNames{
                    ...
                    let funcnameoffset = self.rvatofileoffset(funcnameoffset as usize).unwrap();
    ...
    pub fn getbaserelocations(&self) -> Result<Vec<HashMap<u32,u16>>,String> {
        unsafe{

            if self.filecontents.len()>1{
                let mut relocations: Vec<HashMap<u32,u16>> = Vec::new();

                let ntheader = self.getntheader();
                if ntheader.OptionalHeader.DataDirectory[5].Size==0{
                    return Err(format!("no base relocations are present"));
                }
                ...
                loop{
                    ...
                    for i in 0..entriescount{
                        let mut h1:HashMap<u32,u16> = HashMap::new();
                        let valueoffset = std::ptr::read(((baserelocptr as usize + 8) + (i as usize * 2)) as *const u16);
                        if valueoffset>>12 == 0xA{
                            //  println!("pagerva: {:x?}, offset to relocate: {:x?}",
                            //  basereloc.VirtualAddress,valueoffset&0x0fff);
                            h1.insert(basereloc.VirtualAddress,
                                      valueoffset&0x0fff);
                            relocations.push(h1);
                        }

                    }
                    //println!("baserelocptr: {:x?}",baserelocptr);
                ...
                let mut baserelocptr = self.memorycontents.as_ptr() as usize + ntheader.OptionalHeader.DataDirectory[5].VirtualAddress as usize;
